...
treeAI - локальное desktop-приложение для диалогов с AI, где разговоры живут не одной длинной лентой, а деревом. Можно вести несколько направлений мысли параллельно, создавать ветки от выбранного узла, возвращаться к развилкам и не терять контекст.
...
Обычный чат быстро превращается в длинный туннель: один вопрос цепляется за следующий, альтернативные идеи теряются, а откат к старой мысли ломает контекст. treeAI решает это через визуальное дерево:
...
```text
~/Library/Application Support/treeAI/treeai.sqlite3
```
...
- Убран дублирующий header внутри чат-панели: название выбранной ветки и дерево теперь показываются в верхней toolbar вместо статичного `treeAI`, освобождая вертикальное место под сообщения.
